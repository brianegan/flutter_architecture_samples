...
main() {
  group('TodoListModel', () {
    test('should check if there are completed todos', () async {
      ...
      expect(model._filteredTodos.any((it) => it.complete), true);
    });
    ...
    test('should calculate the number of active todos', () async {
      ...
      expect(
          model._filteredTodos.where((it) => !it.complete).toList().length, 2);
    });
    ...
    test('should calculate the number of completed todos', () async {
      ...
      expect(
          model._filteredTodos.where((it) => it.complete).toList().length, 1);
    });
    ...
    test('should return all todos if the VisibilityFilter is all', () async {
      ...
      expect(model._filteredTodos, todos);
    });
    ...
    test('should return completed todos if the VisibilityFilter is completed',
      ...
      expect(model._filteredTodos, [todo3]);
    });
    ...
    test('toggle all as complete or incomplete', () async {
      ...
      // Toggle all complete
      model.toggleAll();
      expect(model._filteredTodos.every((t) => t.complete), isTrue);
